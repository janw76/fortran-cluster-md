...
*	READ XYZ-FILE
*===============================================================================
* This routine reads the positions of the different particles from the
* XYZ-file.
...
*-------------------------------------------------------------------------------
	...
	character*2 aslst(mxa),asnew,astemp
...
*-------------------------------------------------------------------------------
	...
	if (nat.gt.natom) then
	  do i=1,nat-natom
	    read(fnum,10) astemp,x(1),y(1),z(1)
	  enddo
	endif
	...
	do i=1,natom
	  read(fnum,10) aslst(i),x(i),y(i),z(i)
	  x(i)=x(i)+hx
	  y(i)=y(i)+hy
	  z(i)=z(i)+hz
	enddo
