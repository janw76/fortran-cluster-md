*===============================================================================
* 	4th ORDER GEAR-INTEGRATOR
*===============================================================================
* file: int-gear4.f
*-------------------------------------------------------------------------------
* 4th order Gear algorithm. The factors Fx2 are calculated for minimum
...
*-------------------------------------------------------------------------------
	...
	double precision f02,f12,f22,f32,f42
...
*-------------------------------------------------------------------------------
	...
	do i=1,natom
	  x0(i)=x0(i)+x1(i)*dt1+x2(i)*dt2+x3(i)*dt3+x4(i)*dt4
	  y0(i)=y0(i)+y1(i)*dt1+y2(i)*dt2+y3(i)*dt3+y4(i)*dt4
	  z0(i)=z0(i)+z1(i)*dt1+z2(i)*dt2+z3(i)*dt3+z4(i)*dt4
	  x1(i)=x1(i)+x2(i)*dt1+x3(i)*dt2+x4(i)*dt3
	  y1(i)=y1(i)+y2(i)*dt1+y3(i)*dt2+y4(i)*dt3
	  z1(i)=z1(i)+z2(i)*dt1+z3(i)*dt2+z4(i)*dt3
	  x2(i)=x2(i)+x3(i)*dt1+x4(i)*dt2
	  y2(i)=y2(i)+y3(i)*dt1+y4(i)*dt2
	  z2(i)=z2(i)+z3(i)*dt1+z4(i)*dt2
          x3(i)=x3(i)+x4(i)*dt1
	  y3(i)=y3(i)+y4(i)*dt1
	  z3(i)=z3(i)+z4(i)*dt1
	  axp(i)=x2(i)
